...
# For calculate total time outside (person left the room)
# def calculate_total_time_outside(tracked_id):
#     if tracked_id in person_exit_time:
#         exit_time = person_exit_time[tracked_id]
#         time_outside = datetime.now() - exit_time
#         if tracked_id not in total_time_outside:
#             total_time_outside[tracked_id] = 0
#         if tracked_id not in num_exits:
#             num_exits[tracked_id] = 0
#         total_time_outside[tracked_id] += time_outside.total_seconds()
#         num_exits[tracked_id] += 1
#         print(f"Person {tracked_id} has been outside the room for "
#               f"{time_outside.total_seconds()} seconds")
#
#         if time_outside.total_seconds() > 5:  # 600 seconds = 10 minutes
#             print(f"WARNING: Person {tracked_id} has been outside the room for more than 10 minutes!")
#
...
#             previous_exit_time[tracked_id] = exit_time
#
#         print(f"Total time outside the room for person {tracked_id}: "
#               f"{total_time_outside[tracked_id]} seconds")
#         print(f"Person {tracked_id} has left the room {num_exits[tracked_id]} times.")
#
#         person_exit_time[tracked_id] = None
#     else:
#         print(f"Person {tracked_id} has no exit time recorded.")
...
def total_time(tracked_id, schedule_start_time, schedule_end_time):
    ...
    # Late time
    late_time = count_late_time(person_id, scheduled_start_time)
    # Total time outside
    # total_time_outside = count_left_time(person_id)
    ...
    total_time = scheduled_duration - late_time - total_time_outside[tracked_id]
            ...
            if success:
                ...
                if x + w > RIGHT_ZONE and previous_capture_status.get(tracked_id) != 'left':
                    ...
                    if schedule_times:
                        ...
                        total_time(tracked_id, schedule_start_time, schedule_end_time)
                    ...
                    # if tracked_id not in person_exit_time:
