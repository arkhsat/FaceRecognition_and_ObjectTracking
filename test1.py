        ...
        for i, (tracker, tracked_id) in enumerate(zip(trackers, tracked_ids)):
            ...
            if success:
                ...
                if x + w > RIGHT_ZONE and previous_capture_status.get(tracked_id) != 'left':
                    ...
                    display_image_from_url(image_url)
                    person_exit_time[tracked_id] = datetime.now()
                    previous_exit_time[tracked_id] = person_exit_time[tracked_id]
                    ...
                    if tracked_id in person_exit_time:
                        ...
                        if time_outside.total_seconds() > 6:  # 600 detik = 10 menit
                        ...
                        if tracked_id in previous_exit_time:
                            total_time_outside[tracked_id] += (exit_time -
                                                               previous_exit_time[tracked_id]).total_seconds()
                    ...
                    #
                    #     # Warning more than 10 minute
                    #     if total_time_outside[tracked_id] / 60 >= 10:
                    #         print("Lecture is out more than 10 minutes. CALL THE LECTURE")
                #
                #         # Warning if lecture not enter room
                #         no_entry = datetime.scheduled_duration()
                #         if no_entry > 10:
                #             print("Lecture not entter the room. CALL THE LECTURE")
